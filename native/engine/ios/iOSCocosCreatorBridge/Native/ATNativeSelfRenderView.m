...
//        CGRect frame = CGRectFromString(metrics[key][kParsedPropertiesFrameKey]);
        ...
        CGRect frame = CGRectMake([metric[@"x"] doubleValue] / scale, [metric[@"y"] doubleValue] / scale, [metric[@"width"] doubleValue] / scale, [metric[@"height"] doubleValue] / scale);
        NSLog(@"🔥AnythinkDemo::nativekey-%@,Frame-%@",key,NSStringFromCGRect(frame));
        obj.frame = frame;
        
        if ([obj respondsToSelector:@selector(setBackgroundColor:)] && [metrics[key] containsObjectForKey:@"background_color"]) {
